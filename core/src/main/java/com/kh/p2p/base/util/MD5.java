...
/**
 * MD5 算法的Java Bean MD5 类实现了RSA Data Security, Inc. 在提交给IETF
...
public class MD5 {
	...
	public synchronized String getMD5ofStr(String inbuf) {
		...
		md5Final();
		digestHexStr = "";
		digestHexStr += byteHEX(digest[8]);
		digestHexStr += byteHEX(digest[13]);
		digestHexStr += byteHEX(digest[7]);
		digestHexStr += byteHEX(digest[3]);
		digestHexStr += byteHEX(digest[1]);
		digestHexStr += byteHEX(digest[5]);
		digestHexStr += byteHEX(digest[11]);
		digestHexStr += byteHEX(digest[4]);
		return digestHexStr;
	}
	...
	/*
	 * F, G, H
	 * ,I是4个基本的MD5函数，在原始的MD5的C实现中，由于它们是简单的位运算，可能出于效率的考虑把它们实现成了宏
	...
	/*
	 * FF,GG,HH和II将调用F,G,H,I进行近一步变换FF, GG, HH, and II transformations
	 * for rounds 1, 2, 3, and 4. Rotation is separate from addition
